...
<head>
    ...
    <style>
        * { font-family: DejaVu Sans, sans-serif; }
        ...
        .flow { font-size: 14px; font-weight: bold; }
...
<body>
    <h1>Zmiana Miejsca Użytkowania (ZMU)</h1>
    <div class="muted">
        Dokument nr ZMU-{{ $request->id }} · wygenerowano {{ now()->format('Y-m-d H:i') }}
    </div>
    ...
    <div class="box">
        <table>
            <tr>
                <th>Pole spisowe źródłowe</th>
                <td class="flow">{{ $request->sourceField?->label() ?? '—' }}</td>
            </tr>
            <tr>
                <th>Pole spisowe docelowe</th>
                <td class="flow">{{ $request->targetField?->label() ?? '—' }}</td>
            </tr>
        </table>
    </div>
    ...
    <div class="box">
        <table>
            ...
            @if ($request->note)
                <tr><th>Notatka</th><td>{{ $request->note }}</td></tr>
            @endif
        </table>
    </div>
    ...
    <table class="sign">
        <tr>
            <td>Podpis zdającego</td>
            <td class="spacer"></td>
            <td>Podpis przyjmującego</td>
        </tr>
    </table>
</body>
</html>
