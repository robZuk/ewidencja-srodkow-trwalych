...
new #[Layout('components.layouts.app', ['title' => 'Druki ZMU'])] class extends Component
{
    use WithPagination;
    ...
    public function with(): array
    {
        return [
            'requests' => TransferRequest::query()
                ->ofType(TransferType::Transfer)
                ->with(['asset', 'sourceField', 'targetField'])
                ->latest()
                ->paginate(15),
        ];
    }
...
<div>
    <div class="mb-6">
        <flux:heading size="xl">Druki ZMU</flux:heading>
        <flux:subheading>Zmiana Miejsca Użytkowania — generowanie dokumentów przekazań.</flux:subheading>
    </div>
            ...
            <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800/70">
                @forelse ($requests as $request)
                    <tr wire:key="zmu-{{ $request->id }}">
                        ...
                        <td class="px-4 py-3 text-zinc-500">{{ $request->sourceField?->code }} → {{ $request->targetField?->code }}</td>
                        ...
                        <td class="px-4 py-3 text-right">
                            <flux:button :href="route('druki.zmu.pdf', $request)" size="xs" variant="subtle" icon="arrow-down-tray">PDF</flux:button>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="5" class="px-4 py-12 text-center text-zinc-500">Brak przekazań.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
